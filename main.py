...
def get_youtube_data(api_key: str, title: str) -> dict:
	"""Retrieve view count of a trailer from YouTube."""
	youtube = build('youtube', 'v3', developerKey=api_key)
	youtube_data = []
	trailer = youtube.search().list(part='id', q=f'{title} trailer', maxResults=1).execute()
...
def get_myanimelist_data(title: str) -> dict:
	"""Gather scoring data from MyAnimeList."""
...
def create_myanimelist_table(myanimelist_data: dict, cur, conn):
	...
	cur.execute("""CREATE TABLE IF NOT EXISTS MyAnimeList (
		...
		dropped INTEGER, plan_to_watch INTEGER)""")
	...
	cur.execute("""INSERT INTO MyAnimeList (
		...
		on_hold, dropped, plan_to_watch) VALUES 
		(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
		(max_id + 1, title, score, rank, popularity,
		 audience, voters, watching, completed, on_hold, 
		 dropped, plan_to_watch))
	conn.commit()


def table_exists(cur, conn) -> tuple:
...
if __name__ == '__main__':
	...
	client_id = os.getenv('POETRY_MAL_CLIENT_ID')
	client_secret = os.getenv('POETRY_MAL_CLIENT_SECRET')
	if st.button('Get Auth Code'):
		code_verifier = code_challenge = get_new_code_verifier()
		open_authorization_link(code_challenge, client_id)
		os.environ['POETRY_MAL_CODE_VERIFIER'] = code_verifier
	
	authorization_code = st.text_input('Auth Code')
	if authorization_code:
		os.environ['POETRY_MAL_AUTH_CODE'] = authorization_code
		st.text('Authorization code saved.')
	
	if st.button('Refresh Token'):
		code_verifier = os.getenv('POETRY_MAL_CODE_VERIFIER')
		authorization_code = os.getenv('POETRY_MAL_AUTH_CODE')
