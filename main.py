...
def get_youtube_data(api_key: str, title: str) -> dict:
	...
	youtube_data = []
	video = youtube.search().list(part='id', q=f'{title}', maxResults=1, order='viewCount').execute()
...
def create_myanimelist_table(myanimelist_data: dict, cur, conn):
	...
	air_day = myanimelist_data['day_of_the_week']
	air_time = myanimelist_data['start_time']
